...
task<> application::forward_message(uint32_t id, const memory_buffer& message) {
    ...
    for (const auto results = co_await wait_parallel_task_ready(parallel_task_type::wait_all, tasks);
...
void application::awake_services() {
    ...
    try {
        sync_wait([this, &fail_name]() -> task<> {
            ...
            size_t index = 0;
            for (const auto results = co_await wait_parallel_task_ready(parallel_task_type::wait_one_fail, tasks);
                 const auto& result : results) {
                if (const auto e = result.get_exception()) {
